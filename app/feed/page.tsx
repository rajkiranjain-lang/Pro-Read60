...
export default async function FeedPage() {
  ...
  const following: FollowRow[] = await db.follow.findMany({ where: { followerId: user.id }, select: { followingId: true } });
  const ids = [user.id, ...following.map((x: FollowRow) => x.followingId)];
  ...
  return <main className="mx-auto min-h-screen max-w-2xl border-x border-zinc-200 bg-white"><header className="sticky top-0 z-10 border-b bg-white/90 px-5 py-4 backdrop-blur"><h1 className="text-xl font-black">Home</h1></header><Composer /><section>{posts.length ? posts.map((post: (typeof posts)[number]) => <PostCard key={post.id} post={post} />) : <div className="p-10 text-center text-zinc-500">No posts yet. Follow people or publish your first post.</div>}</section></main>;
}
