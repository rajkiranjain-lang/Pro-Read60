...
async function main() {
  ...
  const admin = await db.user.upsert({ where: { email: "admin@pro-read60.local" }, update: {}, create: { username: "admin", email: "admin@pro-read60.local", passwordHash, role: UserRole.SUPER_ADMIN, emailVerifiedAt: new Date(), profile: { create: { displayName: "Pro-Read60 Admin" } }, preferences: { create: {} }, notificationPreferences: { create: {} } } });
